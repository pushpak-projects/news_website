...
class Sports extends Component {
    ...
    componentDidMount() {
        try {
            console.log("inside sports.js class")

            console.log("This is Guardian")

            axios.get(`https://p2u1s1h1a1k1hw8-node-final1.wl.r.appspot.com/news/guardianSection?section=sport&apiKey=d6d06dbc-9431-4c6f-a539-18e9e6f6fbf7`)
                .then((response) => {
            ...
            axios.get(`https://p2u1s1h1a1k1hw8-node-final1.wl.r.appspot.com/news/nytimes?section=sports&apiKey=xFICtv25WL2BRKqP1YpIGlFhHXbK9CoG`)
                .then((response) => {
                    console.log(response)
        ...
        if (this.state.isLoading) {
            ...
            if (localStorage.getItem('checked') == "true") {
                ...
                for (var i = 0; i < Math.min(dataArr1.length, 10); i++) {
                    if (dataArr1[i].webTitle === "" || dataArr1[i].blocks.body[0].bodyTextSummary === "" || dataArr1[i].webPublicationDate === "" || dataArr1[i].sectionId === "" || dataArr1[i].sectionId !== "sport" || typeof dataArr1[i].blocks.main === 'undefined') {
                        continue;
                    }
                    arr.push(dataArr1[i])
                }
