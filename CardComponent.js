...
class CardComponent extends Component {
    ...
    componentDidMount() {
        try {
            ...
            axios.get(`https://p2u1s1h1a1k1hw8-node-final1.wl.r.appspot.com/news/guardian?apiKey=d6d06dbc-9431-4c6f-a539-18e9e6f6fbf7`)
                .then((response) => {
                    console.log(response)

                    const data = response.data.response.results;
                    this.setState({ dataArrGA: data })
                    this.setState({ isLoading: false })
                });


            axios.get(`https://p2u1s1h1a1k1hw8-node-final1.wl.r.appspot.com/news/nytimes?section=home&apiKey=xFICtv25WL2BRKqP1YpIGlFhHXbK9CoG`)
                .then((response) => {
                    console.log(response)
        ...
        if (this.state.isLoading) {
            ...
            if (localStorage.getItem('checked') == "true") {
                ...
                for (var i = 0; i < dataArr1.length; i++) {
                    if (dataArr1[i].webTitle === "" || dataArr1[i].blocks.body[0].bodyTextSummary === "" || dataArr1[i].webPublicationDate === "" || dataArr1[i].sectionId === "") {
                        continue;
                    }
                    arr.push(dataArr1[i])
                }
